...
export function CharacterCard({ character, alias, personal = false }: {
  ...
  const like = !character.player && !personal && (user ? <fetcher.Form method="post" action={'/game/' + alias + '/roles'}>
  ...
  return <article className={'character' + (personal ? ' character--personal' : '')} id={'character-' + character.id}>
    {!personal && <div className="character-mobile-title"><span>{character.alias}</span><div>
      ...
      {character.player ? <details className="player-popup"><summary>{character.player.username}</summary><SocialLinks player={character.player} /></details> : <span>Свободно</span>}
    </div></div>}
    ...
    {!personal && <div className="character-player"><span>{character.player?.username || 'Свободно'}</span>
