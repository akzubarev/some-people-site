<template>
  <div>
    ...
    <div class="ml-2 mt-3 flex">
      ...
      <span
          class="px-4 border text-xl border-teal-600 text-teal-600 hover:bg-teal-600 hover:text-white focus:outline-none focus:shadow-outline"
      >
            <slot></slot>
          </span>
      <button
          @click="progress = Math.min(progress+interval,max); $emit('change', progress)"
          class="px-4 border text-xl border-teal-600 border-l-0 text-teal-600 hover:bg-teal-600 hover:text-white rounded-r focus:outline-none focus:shadow-outline"
      >+
      </button>
    </div>
  </div>
