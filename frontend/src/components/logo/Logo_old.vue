<template>
  <span class="Logo" ref="target">DIVIDENDO</span>
</template>
<script>
...
export default {
  ...
  setup() {
    ...
    watch(target, () => {
      ...
      const flickerLetter = letter => `<span ">${letter}</span>`
      ...
      neonGlory(target.value)
      target.value.onclick = ({ target }) => neonGlory(target)
    })
    return {
      target
    }
